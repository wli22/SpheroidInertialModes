#ifndef PROLATEMODE_HPP
#define PROLATEMODE_HPP
...
class ProlateMode {
    public:
        ProlateMode(const int m, const int k, const int delta, const T eccentricity);

        ProlateMode(const ProlateMode<T>& other)
        ...
        ProlateMode(ProlateMode<T>&& other) noexcept
        ...
        ProlateMode<T>& operator=(const ProlateMode<T>& other) {
            if (this != &other) {
                m_            = other.m_;
                m_abs_        = other.m_abs_;
                k_            = other.k_;
        ...
        ProlateMode<T>& operator=(ProlateMode<T>&& other) noexcept {
            if (this != &other) {
                m_            = other.m_;
                m_abs_        = other.m_abs_;
                k_            = other.k_;
...
template <typename T>
ProlateMode<T>::ProlateMode(const int m, const int k, const int delta, const T eccentricity) : m_(m), m_abs_(std::abs(m)), k_(k), delta_(delta), l_(m_abs_+2*k_+delta_), eccentricity_(eccentricity) {
...
T ProlateMode<T>::sigma_equation(const T sigma) const {
    ...
    if (m_ != 0) {
        result *= (T(1)-sigma2)*one_sub_ecc2/(std::sqrt(T(1)-xi*xi)*scale);
        result -= static_cast<T>(m_)*plm;
    }
...
template <typename T>
T ProlateMode<T>::bisection_search(const T sigmal, const T sigmar) const {
...
template <typename T>
void ProlateMode<T>::sigma_initialization() {
...
// template <typename T>
// std::pair<T, T> ProlateMode<T>::get_X_and_Y(const T eta, const T tau, const T sigma) const {
//     std::pair<T, T> XY;
...
//     T AlphaBeta = std::sqrt(static_cast<T>(1)-ecc2+ecc2*sigma2) / sigma;
...
std::pair<T, T> ProlateMode<T>::get_X_and_Y(const T eta, const T tau, const T sigma) const {
    ...
    long double AlphaBeta = std::sqrt(1.0L-ecc2+ecc2*sigma2) / sigma;
...
template <typename T>
std::tuple<T, T, T> ProlateMode<T>::get_terms_123(const T X, const T Y) const {
...
template <typename T>
std::pair<T, T> ProlateMode<T>::pressure_mode(const int n, const T eta, const T tau, const T phi) const {
...
template <typename T>
std::pair<T, T> ProlateMode<T>::velocity_mode_eta(const int n, const T eta, const T tau, const T phi) const {
    const T sigma = sigmamnk_.at(n-1);
    const auto[X, Y] = get_X_and_Y(eta, tau, sigma);
    ...
    T one_sub_ecc2_add_ecc2sigma2 = static_cast<T>(1)-ecc2+ecc2*sigma2;
    T one_sub_ecc2_add_ecc2sigma2_sqrt = std::sqrt(one_sub_ecc2_add_ecc2sigma2);
    T scale = eta*std::sqrt(static_cast<T>(1)-tau2) / (dv*std::sqrt(one_sub_ecc2_add_ecc2sigma2*one_sub_sigma2));
    if (std::isnan(scale)) scale = static_cast<T>(0);
    kernel = (m_ == 0) ? scale*sigma*term1 : scale*(sigma*term1-term2);
    scale = tau*std::sqrt(eta2-ecc2) / (dv*one_sub_ecc2_add_ecc2sigma2_sqrt);
    if (std::isnan(scale)) scale = static_cast<T>(0);
    kernel -= scale*term3;
...
template <typename T>
std::pair<T, T> ProlateMode<T>::velocity_mode_tau(const int n, const T eta, const T tau, const T phi) const {
    const T sigma = sigmamnk_.at(n-1);
    const auto[X, Y] = get_X_and_Y(eta, tau, sigma);
    const auto[term1, term2, term3] = get_terms_123(X, Y);
    std::pair<T, T> umnk;
    ...
    T one_sub_ecc2_add_ecc2sigma2 = static_cast<T>(1)-ecc2+ecc2*sigma2;
    T one_sub_ecc2_add_ecc2sigma2_sqrt = std::sqrt(one_sub_ecc2_add_ecc2sigma2);
    T scale = tau*std::sqrt(eta2-ecc2)/(dv*std::sqrt(one_sub_sigma2*one_sub_ecc2_add_ecc2sigma2));
    if (std::isnan(scale)) scale = static_cast<T>(0);
    kernel = (m_ == 0) ? scale*sigma*term1 : scale*(sigma*term1-term2);
    scale = eta*std::sqrt(static_cast<T>(1)-tau2)/(dv*one_sub_ecc2_add_ecc2sigma2_sqrt);
    if (std::isnan(scale)) scale = static_cast<T>(0);
    kernel += scale*term3;
    kernel /= static_cast<T>(-2);
    if (m_ == 0) {
...
template <typename T>
std::pair<T, T> ProlateMode<T>::velocity_mode_phi(const int n, const T eta, const T tau, const T phi) const {
    const T sigma = sigmamnk_.at(n-1);
    const auto[X, Y] = get_X_and_Y(eta, tau, sigma);
    const auto[term1, term2, term3] = get_terms_123(X, Y);
    std::pair<T, T> umnk;
    ...
    T ecc2 = eccentricity_*eccentricity_;
    T kernel, scale;
    scale = static_cast<T>(-2)*std::sqrt((static_cast<T>(1)-ecc2+ecc2*sigma2)*(static_cast<T>(1)-sigma2));
...
template <typename T>
std::tuple<T, T, T, T, T, T> ProlateMode<T>::velocity_mode(const int n, const T eta, const T tau, const T phi) const {
    const T sigma = sigmamnk_.at(n-1);
    const auto[X, Y] = get_X_and_Y(eta, tau, sigma);
    const auto[term1, term2, term3] = get_terms_123(X, Y);
    T sigma2 = sigma*sigma;
    ...
    T one_sub_ecc2_add_ecc2sigma2 = static_cast<T>(1)-ecc2+ecc2*sigma2;
    T one_sub_ecc2_add_ecc2sigma2_sqrt = std::sqrt(one_sub_ecc2_add_ecc2sigma2);
    T one_sub_ecc2_add_ecc2sigma2_mut_one_sub_sigma2_sqrt = std::sqrt(one_sub_ecc2_add_ecc2sigma2*one_sub_sigma2);
    ...
    if (m_ == 0) {
        T sigmaterm1scaled = sigma*term1/one_sub_ecc2_add_ecc2sigma2_mut_one_sub_sigma2_sqrt;
        kernel_uradius = scale1*sigmaterm1scaled;
        kernel_uang = scale2*sigmaterm1scaled;
        kernel_uphi = term1/one_sub_ecc2_add_ecc2sigma2_mut_one_sub_sigma2_sqrt;
    } else {
        T tmp = (sigma*term1-term2)/one_sub_ecc2_add_ecc2sigma2_mut_one_sub_sigma2_sqrt;
        kernel_uradius = tmp*scale1;
        kernel_uang = tmp*scale2;
        kernel_uphi = (term1-sigma*term2)/one_sub_ecc2_add_ecc2sigma2_mut_one_sub_sigma2_sqrt;
    }
    kernel_uradius -= term3*scale2/one_sub_ecc2_add_ecc2sigma2_sqrt;
    kernel_uang += term3*scale1/one_sub_ecc2_add_ecc2sigma2_sqrt;
...
template <typename T>
std::pair<T, T> ProlateMode<T>::divergence_velocity_mode(const int n, const T eta, const T tau, const T phi) const {
    const T sigma = sigmamnk_.at(n-1);
    T sigma2 = sigma*sigma;
    ...
    T divergence = (div_X+div_Y+div_phi)/(T(2)*one_sub_sigma2)*std::sqrt(T(1)-eccentricity_*eccentricity_*one_sub_sigma2);
